...
**迷ったら「この文書を読まないと実装を正しく変更できないか」で判断する。**

| 書くもの | 置き場 |
|---------|--------|
| 使い方の手順（利用者向け） | `docs/guide/` |
| 仕様・方針（**直す人が引く**もの） | `docs/reference/` |
| 構造・流れの図 | `docs/diagrams/` |
| **なぜこの設計なのか**（決定は書き換えない） | `docs/background/` |
| **調査・作業指示・レビュー結果・実測記録** | **ProjectTemplete リポジトリ `docs/`**（→ 下記） |

- **`docs/` 直下にファイルを置かない。** 上の4つのどれかに入れる
  （直下に散らばって「目的が分からない」状態になり、2026-08-16 に整理した）
- **本リポジトリは実装だけを持つ。** 経緯・記録は **ProjectTemplete** に残す。
  本リポジトリは public であり、かつ「今どうなっているか」を引く場所である
- **ただし、記録から導かれた結論が実装の前提になっている場合は、結論だけ本リポジトリに残す。**
  例: permissions の実機検証記録は ProjectTemplete へ移したが、
  そこから導かれた**「単純化してはいけない4点」は `docs/reference/` に残した**。
...
> `git add -A` / `git add .` を **deny する**ようにした（→ §9）。
