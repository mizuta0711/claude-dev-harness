...
git commit --amend           # ⚠️ インデックス全体を取り込む（下記）
```

**`git add -A` / `git add .` / パス指定なしの `git commit` は使わない。**

### ⚠️ `git commit --amend` も同じ実害を持つ

**`--amend` はパス指定なしだとインデックス全体を取り込む。** `add -A` / `commit -a` は
deny しているのに、**`--amend` は長く素通りしていた**（H32）。
自分の直前のコミットを直すのは正当なので deny はしないが、**警告は出す**。

> **実際に起きた（2026-08-20）。** パス指定で正しく2ファイルだけコミットした直後、
> **メッセージの誤字を直す `--amend` が、その数秒の間に別セッションが `git mv` で
> ステージしていたリネーム2件を巻き込んだ**。
> **直前のコミットを書き換えるので、巻き込んでも差分に現れず気づきにくい。**
>
> 復旧でもう1つ分かった。**`git add` でステージし直すと `.gitattributes` により
> CRLF が LF へ正規化され、他セッションがステージした blob と変わる**
> （リネーム類似度が R100 → R095 に落ちたのが手がかりだった）。
> 厳密に戻すには `git update-index --cacheinfo <mode>,<blob>,<path>` が要る。

**`--amend` の前に必ず `git status --short` を見る。**
...
| **パス指定なしの `git commit --amend`** | **警告のみ**（H32）。**インデックス全体を取り込む**が、自分の直前のコミットを直すのは正当 |
