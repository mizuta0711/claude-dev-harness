...
- スキルの実体は **`SKILL.md` という手順書**。読み込まれたメイン Claude が、その順に自分で進める
  - **この性質は「保険」でもある。** スラッシュコマンドが解決しないクライアント
    （VS Code の Claude Code 拡張パネルで実測）でも、**`SKILL.md` を読んで従えば同じことができる**。
    パスの引き方と手順は `CLAUDE.md`「スラッシュコマンドが解決しない環境での実行」にある
- **`.claude/rules/` は手動で読まない。** 対象ファイルを読んだ時点で paths 条件により自動ロードされる
- **観点は3層で、毎回全部は見ない。** 共通観点 + プロジェクトの rules + 環境プラグインの観点を、変更内容に応じて使う
- **設計レビュー（`design-review`）だけは4つ目の入力がある。** `harness.config.json` の
  `projectDocs`（要件＝Stage 1 / 設計方針＝Stage 2）は**自動ロードされない**ため、スキルが明示的に読む。
  未登録なら素通りする
