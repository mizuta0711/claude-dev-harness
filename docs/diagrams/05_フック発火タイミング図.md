...
| `pre-adb-uninstall-guard`（android） | PreToolUse | `Bash` / `PowerShell` のうち**アンインストール操作**と判定したもの（`adb uninstall` / `pm uninstall` / `gradlew uninstall*`） | ✅ `deny`（**このアプリのときだけ**。他パッケージ・`-k` 付き・`applicationId` 未設定は警告のみ） |
