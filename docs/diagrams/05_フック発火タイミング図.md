...
| 項目 | 内容 |
|------|------|
| 対応ハーネス版 | harness-core 0.9.2 / harness-nextjs 0.4.2 / harness-unity 0.3.1 / harness-wpf 0.3.2 / harness-android 0.2.1 |
| 最終更新 | 2026-08-19 |
...
| **PreToolUse フック** | `pre-commit-scope` / `pre-commit-check`（core）/ `pre-migrate-backup`（nextjs）。**ブロックできる** |
| **Bash / Git** | 実際のコマンド実行 |
...
> [permissionsベースライン.md §5](../reference/permissionsベースライン.md) にまとめてある
...
| `pre-commit-scope` | PreToolUse | 範囲まるごとの git 操作（`add -A` / `commit -a` / `stash` / 範囲指定なしの破棄） | ⚠️ **既定は警告のみ**。`gates.commitScope: "paths"` のときだけ `deny` |
...
| `pre-adb-uninstall-guard`（android） | PreToolUse | `Bash` / `PowerShell` のうち**ローカルデータを全消去する操作**と判定したもの（`adb uninstall` / `pm uninstall` / `pm clear` / `gradlew uninstall*`。**`:app:` などのモジュール修飾形と、引用符でくくった `adb shell "pm uninstall …"` も含む**） | ✅ `deny`（**このアプリのときだけ**。`applicationIdSuffix` 付きの `<applicationId>.*` も自分のアプリとして扱う。他パッケージ・`-k` 付き・`applicationId` 未設定は警告のみ） |
