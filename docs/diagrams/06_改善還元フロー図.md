...
| **テンプレート層** | `CLAUDE.md` / `constitution.md` / `.claude/rules/` / `.claude/harness.config.json` / `docs/` 骨格 | `/harness-core:harness-update` の3点比較 | なし |
...
flowchart TD
    ...
    subgraph UP["利用側プロジェクトでの取り込み"]
        U1["claude plugin marketplace update dev-harness"]
        U2["関門4: claude plugin update harness-core@dev-harness<br/>--scope project（スコープは project のみにしておく）"]
...
| **4. update のスコープ** | `user` だけ更新され `project` が古いまま残る。**セッションがどちらに解決するかで読み込まれる版が変わる** | 下記のワンライナーで `user` / `project` の版を突き合わせる |
...
`enabledPlugins` によりセッション起動時に project スコープの登録が**自動で作られる**ため、
利用側は放っておくと**`user` と `project` の両方**を持つ。既定の `claude plugin update` は `user` しか更新しない。

> **対策は「`project` だけにする」。** 自動生成される `project` 側は消しても復活するので、
> 捨てられるのは `user` 側だけである。`claude plugin uninstall ... --scope user` を一度実行し、
> 以降の導入・更新は `--scope project` を付けて行う（手順は
> [セットアップガイド §6-1](../guide/セットアップガイド.md#6-1-プラグイン層を更新するskills--agents--hooks)）。
...
| 「起動時に `[harness] environment: nextjs` が出れば導入成功」 | **出ない。** SessionStart フックの出力は `additionalContext` で Claude に渡るもので画面には表示されない。確認は `/plugin` と `/` で行う |
| 「フックが何も言わないから動いていない」 | **`additionalContext` は Claude の文脈には入るが、ユーザーの画面には出ない。** `post-edit-lint` の指摘通知もこの経路を通る（C1 で実測）。フックの動作は表示ではなく、Claude の応答や成果物の変化で判断する |
...
手順の詳細は [plugin-development.md](../plugin-development.md)、変更履歴は [CHANGELOG.md](../../CHANGELOG.md) を参照。
