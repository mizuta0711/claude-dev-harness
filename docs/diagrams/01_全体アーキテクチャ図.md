...
| **harness-core** | 環境非依存の本体。スキル10 / エージェント3 / フック5 |
...
flowchart LR
    subgraph REPO["claude-dev-harness（public・単一ソース）"]
        ...
        subgraph PLUGINS["plugins/ — プラグイン層"]
            CORE["harness-core<br/>skills 10 / agents 3 / hooks 5"]
...
|-----------|--------|-------------|--------|
| `harness-core` | `new-feature` / `design-review` / `code-review` / `build-check` / `update-docs` / `sync-check` / `complete-feature` / `pre-push-check` / `done` / `harness-update` | `coding-specialist` / `code-reviewer` / `documentation-manager` | `session-start-context` / `pre-compact-save` / `pre-commit-check` / `post-commit-doc-check` / `subagent-stop-diff` |
