...
| 自前側 | **claude-dev-harness**（harness-core **0.9.2** / harness-nextjs 0.4.2 / harness-unity 0.3.1 / harness-wpf 0.3.2 / harness-android 0.1.0） |
...
| **動作確認の形式化** | 環境ごとに専用モジュール。**Playwright MCP / Unity MCP（Play モード）/ UIAutomation / adb**。C1 では静的検査を全通過した実装から**本物の不具合を検出した** | **無い** |
| **セッション文脈の保全** | SessionStart（文脈注入）/ PreCompact（コンパクト跨ぎの退避）/ SubagentStop（差分提示） | **無い**（エージェント非依存のため、特定エージェントのライフサイクルに乗れない） |
| **環境非依存の実現方法** | `harness.config.json` という**設定契約**で、Next.js / Unity / WPF / Android を**単一実装**で扱う。新環境の追加は環境アダプタ＋リポジトリ側の登録（手順は `templates/README.md`） | エージェント非依存ではあるが、**開発環境（言語・フレームワーク）の差は扱わない** |
...
| **公式 Claude プラグインが今後も出ない**（agent-agnostic 原則）。第三者製に頼ると依存が二重になる | **実績が偏っている**: 移行の実測は **nextjs 2本 + wpf 1本・いずれも単独作業**。unity / android の既存プロジェクト移行とチーム利用は未検証 |
...
| メンテナ体制の変動歴あり | **未実測の領域が残る** — unity / android 環境の既存プロジェクト移行、チーム利用。実測は単独作業のみ |
