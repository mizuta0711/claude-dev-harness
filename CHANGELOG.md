...
**承認で内容を確認してと言われても何が正しいか判断できない**。現実には見ずに推奨で承認するのでは。
それで承認を挟む意味はあるのか」。**正しい。** 分類ごとに見ると、どこが形骸化しているかがはっきりした。

| 分類 | 人は何を判断できるか |
|---|---|
| `template-improvement`（A≠B かつ **A=C**） | **何もできない。** 定義上**プロジェクトが一切触っていないファイル**で、守るべきローカルの意図が無い。残る問いは「ハーネスの変更は正しいか」だけで、それは**ハーネス側で査読・テスト・validate を通した話** |
| `conflict` | **できる。**「このプロジェクトは意図的にこうしている。戻す／残す／統合する」 |
| `template-removed` | **できる。**「今も使っているか」 |
| `schemaVersion` の引き上げ | **できる。** 実際のマイグレーション |

**判断できない問いに承認を求めると、推奨をそのまま通す儀式になる。**
そして「読まずに承認する」習慣は、**本当に判断が要る `conflict` にも波及する**
（H19 / H23 / H33 と同じ形 — 人の注意力に依存する仕組みは人が次に破る）。

### 変えたこと

**① `template-improvement` は自動適用にした。** 承認を求めない。
適用一覧と **`git diff` で確認できる／`git checkout -- <path>` で戻せる**ことを報告に出す。
**本当の安全網は承認ではなく git** で、1行の要約に「はい／いいえ」を答えるより情報量も回復力も上。
大原則「ローカル改変を無断で上書きしない」には抵触しない（**A=C のものだけが該当する**ため）。

**② 判断が要るものは、人に聞く前に別エージェントの査読を通す**（利用者の提案）。
`conflict` / `template-removed` / `schemaVersion` が対象で、**0件なら査読を起動しない**。
メインが材料（A/B/C・`git log -- <path>`・参照元 grep）を集め、
**結論を渡さずに**サブエージェント（`model: sonnet` 明示。専用エージェントは作らない）へ出す。
**メインが査読の主張を現物で裏取り**してから（§4「査読結果自体も誤りうる」）、
査読の判定・メインの判定・**一致／相違**・推奨・**何が変われば答えが変わるか**を提示する。

> 🔴 **査読とメインが食い違ったら、必ずそう言う。揃えて見せない。**
> **食い違いこそが「人が判断すべき」という信号。**
> 揃えて1つの推奨だけ出すと、**承認が新しい判子になる**（形骸化を一段上へ移しただけになる）。

**③ 「ハーネス更新して」の入口を `harness-update` に一本化した。**
Step 0 で `plugin-update` の手順を通し、そのままテンプレート層まで面倒を見る。**再起動は最後の1回**。

**④ スクリプトは「更新後の版」から引く。** プラグインのキャッシュは**版ごとに別ディレクトリ**なので、
`${CLAUDE_SKILL_DIR}` は**起動時に読み込んだ旧版を指したまま**になる（0.16.0 の説明で私が誤っていた点）。
`plugin-versions.mjs --skill harness-update` で更新後の版を解決し、その `harness-diff.mjs` を使う。
**手順文（SKILL.md）だけは1回遅れる** — 3点比較と分類の実体はスクリプト側なので許容する。

`plugin-update` は**プラグイン層だけを更新したいと分かっているとき**用として残す。

docs 影響: あり（diagrams/01・guide/セットアップガイド・guide/運用ガイド・background/01・
README.md・templates/base/CLAUDE.md）

## [0.16.0] — テンプレート層の追従を「言えば動く」ようにし、放置を起動時に知らせる

**「ハーネス更新して」では動かなかった。** ユーザーが `/harness-core:harness-update` と
打つしかなく、しかも**更新があること自体を誰も知らせていなかった**。

### `harness-update` の `disable-model-invocation` を外した

掲げていた理由は「破壊的・不可逆な操作を含むため、**人の明示的な指示を要件にする**」だったが、
**「ハーネス更新して」は人の明示的な指示そのもの**である。
あのフラグは **「明示的な指示」と「スラッシュコマンド」を取り違えていた。**

**破壊的な部分はスキルの中で既にゲートされている** — 大原則が
「プロジェクト側のローカル改変を無断で上書きしない」「適用はファイル単位でユーザーの承認を取る」
と定めており、Step 1 の `analyze` は一時クローンして比較するだけの**読み取り専用**。
**フラグはその読み取りすら塞いでいた。** 同じ危険度の `plugin-update` に付いていない非対称でもあった。

**`complete-feature` は残す。** あちらはフロー末尾のゲートで、通した時点で設計書が `completed/` へ移る。

### 前回の追従からの経過を起動時に知らせる（`update.intervalDays`）

**テンプレート層は届いていないことに症状が出ない。** プラグイン層は marketplace が運ぶが、
`CLAUDE.md` / `rules/` / `harness.config.json` / docs 骨格は**生成時のコピー**なので、
追従を叩かない限り改善が伝播しない。**気づく手がかりが無かった。**

`audit.intervalDays` と同じ形にした（未設定 30 日 / `0` で無効 / 知らせるだけで止めない）。
**前回日は `harness-baseline.json` の `appliedAt`** — `harness-diff.mjs` の `finalize` が
追従のたびに更新するため、**専用のマーカーは要らない**。

実フックへ payload を流して4パターンを確認した（2日前→出ない／80日前→出る／
`intervalDays: 0`→出ない／`intervalDays: 7`→出る）。

docs 影響: あり（diagrams/03 — `disable-model-invocation` が付くのは `complete-feature` のみに／
reference/harness設定契約 — `update.intervalDays` を追加）

## [docs] — 仕分けの基準を「後継があるか」に変える（H20 / H22 / H25 / H26）

移行指示書 §3 に集まっていた4件を還元した。**別々に起票されたが、根はひとつだった。**

### 基準と手段を分ける（H20 / H25）

3分類は「旧テンプレートの履歴に一致があるか」で判定していたが、
**それは検出の手段であって基準ではない**。混ぜていたため2つの場面で破綻していた。

| 破綻 | 何が起きるか |
|---|---|
| **還元でハーネスへ渡したもの**（H20） | 旧テンプレに無い → 「固有 → 残す」と分類されるが、**後継はハーネスにある**。還元元は必ずこの形になる |
| **祖先が無いプロジェクト**（H25） | blob 照合の相手が存在しない（android には旧テンプレートが無い）。**3分類のうち2つが定義できない** |

基準を **「いまのハーネスに後継があるか」** の一点にし、`§3-1` を「後継を突き止める」に改めて
**blob 照合を「祖先があるときの手段」へ格下げ**した。祖先が無い場合は名前と役割で探す
（実測: SimplePhone で削除4本すべての後継を追跡でき、移送漏れ0件）。

### 後継がある＝全部移った、ではない（H20 後半 / H22）

`§3-1b` を新設。**後継は同じ役割を果たすが、同じ内容を持つとは限らない。** 実測で3形:

- **後継が「意図的に持たない」と宣言している** — 汎用のハーネス版が、プロジェクト固有の用語規約を持たない設計だった（移送漏れ1件）
- **後継の本文が、そのプロジェクトに無い名前を指している**（H22-①） — `rules/` 本文の設計書名は **`{{}}` ではないので `create-project` が置換しない**。「存在しない設計書へ追記せよ」という指示が残る
- **設定は移ったが発火条件が移っていない**（H22-②） — 「`hooks` ブロックは消す」だけを守ると、そのプロジェクト固有の発火条件が `docTriggers` へ移らず消える

**削除前に「この中身は後継のどこへ行くか」を1本ずつ言えること**を条件にした。
言えないものが1つでもあれば、それが移送漏れになる。§4 にも H22 の2件の具体手順を足した。

### 「メインの見立て」列を削除（H26）

`§3-6` は「**査読者に結論を渡さない。渡すのは対象と根拠まで**」と定めながら、
**同じ節のテンプレートに「メインの見立て」列（「消す寄り」）があった**。査読者が実際に踏んだ。

代わりに **「正本との重なり（実測）」** 列を置いた。
**判断材料は厚くするほど査読の質が上がるが、判断そのものを書くと追認しか返ってこない。**

docs 影響: あり（guide/既存プロジェクト移行指示書 §3・§3-1・§3-1b・§3-6・§4・§7 と改訂履歴 1.13）

## [guard] — `git commit --amend` を警告する。区切りの `--` 判定も直す（H32）

`repo-guard` は `git add -A` / `git commit -a` を deny するのに、
**`git commit --amend` は素通りしていた**。`--amend` も**パス指定なしだとインデックス全体を
取り込む**ので、実害は同じである。
...
自分の直前のコミットを直すのは正当な操作なので **deny はせず警告**にした
（パス指定なしの `git commit -m` と同じ扱い）。ステージ中のものを一覧で添える。

**あわせて、区切りの `--` 判定の欠陥を直した。** `args.includes("--")` で見ていたため、
**`--amend` や `--no-verify` のような長いオプション名の中の `--` に誤ヒット**していた。
そのため `isUnscopedCommit` も **`git commit --no-verify -m x` で鳴っていなかった**。
単独トークンであることまで見る `hasPathspecSep` に置き換えた。

> **この欠陥自体が H32 と同根。** 「`--` があるからパス指定済み」という素朴な判定が、
> **オプション名の中の `--` を区別できていなかった**。

テスト11ケースで固定した（鳴るべき5・鳴ってはいけない6）。88件（87 → 88）。
`.claude/hooks/repo-guard.js` は**2箇所にコピーがある**ので ProjectTemplete 側にも反映し、
`diff -q` で一致を確認した。`CLAUDE.md` §1 にも `--amend` の記述を追加している。

docs 影響: なし（`CLAUDE.md` は docs/ ではないので申告対象外だが、§1 と §9 の表を更新した）
...
**文章で**書いたが、**それでは拘束力が無い**という指摘を受けた。正しい。
H19（規約は書いても守られない）・H23（明文化しても破られた）とまったく同じ形で、
このリポジトリが3回学んだ結論に反していた。

**しかも書いた場所の半分は届かない。** `docs/guide/運用ガイド.md` は
**ハーネスのリポジトリにしか無く、プロジェクトへ配られない**。
違反が起きるのは各プロジェクトのセッションの中で、そこからは見えない。

**やり方を持つスキルにした。** `complete-feature` が `docs/features/` に対して
果たしているのと同じ役割を、`receive-handoff` が `docs/handoff/` に対して果たす。
置き場のルールを「覚えている規約」ではなく「**通せばそうなる**」形にする。

中心は **Step 3 の事実確認**である。引き継ぎ文書は前の担当が書いたもので、
**書いた時点で既に古いことがある**。移した先は「未着手の課題の登録簿」のような正本になるため、
**確かめずに移すと誤りが正本に固定され、次に誰かが実装しようとするまで気づかれない**
（実測: あるプロジェクトで、実装済みの機能が「未実装」として起票されていた。
削除元の記録が古いのを、確かめずに新しい正本へ移していた）。

手順は 裏取り → ユーザー確認 → 仕分け（すぐ着手／保留／記録のみ／落とす）→ 移動 → handoff を空に、
の順。**判断（やるかどうか・優先度・外部影響）は勝手に決めない。**

`disable-model-invocation` は**付けていない**。「handoff のこれを受け取って」という
自然文から起動できる必要があるため（`complete-feature` は付いているが、あちらはフロー末尾のゲート）。

**起動時通知も足した**（`session-start-context.js`）。ただし**これは備忘であって拘束力の本体ではない**。
handoff へ入れるのはユーザーの指示なので、呼ぶ契機はユーザー自身が持っている
（`proofread-ja` がゼロ発火だった条件とは違う）。**画面には出さない** — #23 の
「画面には1行だけ。詳細は Claude が使う情報」に従い、handoff は例外条件
（黙って全機能が素通りする／気づかないほど悪化する）に当たらないため。

**載せなかった文書と理由**（`grep` で候補は出したうえでの判断）: `guide/入門ガイド`（初学者向けで、
これは日常操作ではない）／`diagrams/02 開発フロー図`（機能開発のフローに属さない）／
`reference/harness設定契約`（config のキーを消費しない。同文書の例外規定どおり）／
`background/02 SpecKitとの差異`（機能比較の趣旨に合わない）。

検証: `node --test "tests/*.test.mjs"` 86件成功（wiring テストが CLAUDE.md への列挙を強制）/
`claude plugin validate . --strict` 通過 / 実フックへ payload を流し、handoff が1件のとき通知が出て
0件のとき出ないことを確認 / 数え上げ（skills 14 / agents 5 / hooks 7）が実数と一致することを再検査。

docs 影響: あり（background/01・diagrams/01・README.md・guide/運用ガイド・templates/base/CLAUDE.md）

## [templates] — `docs/handoff/` は受け渡し専用であることを明文化する

配置表は `docs/handoff/` を「セッション／担当をまたぐ引き継ぎ文書」と定義していたが、
**受け取った側が何をすべきか**を書いていなかった。その結果、対応中のものが置いたままになる。

**受け取ったら着手前に適切なフォルダへ移し、移してから作業する。handoff 側は残さない。**
`pending/` に対する「保留の置き場であって作業場所ではない」と同じ形の間違いが起きる。

**なぜ書くか（実例）**: 「中身を適切な場所へ移した」という作業が、記録の上で
**「`docs/handoff/` を廃止した」に化けた**。しかも別の文書がそれを引き継いで広め、
**配置表の定義は生きているのに、廃止されたと書いた記録が2つ**という状態になった。
次に読む人（人でも AI でも）が「handoff は使えない」と誤って判断する材料になる。
**置き場が空であること＝廃止ではない。** 空にしてよいのは受け渡し専用だからである。

プラグインの挙動は変わらないので**版は据え置き**（テンプレート層のみ。
既存プロジェクトは `harness-baseline.json` の `templatesCommit` で差分を検出できる）。

docs 影響: あり（guide/運用ガイド 6-2b を新設。`pending/` の 6-2 と並べた）
...
14箇所で使っていたが、**`fable` / `opus` / `sonnet` の3モデルとも1件も検出できなかった**
（ProjectTemplete `docs/reviews/20260817_仕組み自体の要否監査.md` §10）。
うち1モデルは「用語（…／**窓**／…）ともよく統制されており」と**統制された用語として扱った**。

人間の読み手の反応は「**窓ってなに？**」だった。

### 判定基準が間違っていた

当初「直訳語かどうか」「繰り返されているか」で見ていたが、**どちらも関係なかった**。

> **ドキュメントは、前提知識が無くてもそれ単体で理解できることが基本である。**
> 初出の語には**一般的な語義を当てて意味が通るか**を確かめる。通らなければ欠陥。

「窓」は初出の時点で一般語義（＝開口部）が通らない。**そこで確定していた。**
3モデルとも**文脈から「時間の区間」だと推測できてしまった**ために通した。
**推測できることと理解できることは違う。**

### Changed

- **`agents/japanese-proofreader.md`** — 冒頭に**「最優先の基準: 前提知識が無くても読めるか」**を新設。
  「そういう用語なんだろう」で通すことを**最大の失敗モード**として明示し、
  **文脈から推測できた／繰り返されている／専門的な文書だから、はいずれも理由にならない**と書いた
- 7分類の **7（意味の取れない直訳名詞）の判定を、この基準へ寄せた**
- **`templates/*/.claude/rules/docs.md`** — 書く側にも同じ原則を追加
- **`templates/unity/.claude/rules/docs.md`** を新規追加。**unity だけ `docs.md` を持っておらず、
  設計ドキュメントの規約が届いていなかった**（他3環境と揃えた）

### Added — 用語集の門番

**判断を照合に変える**: 用語集に載っていれば固有語、載っていなければ一般語。用語集が無ければ全部一般語。

ただし**用語集は放っておくと際限なく増える**ため、追加にゲートを置く。

- **`agents/glossary-keeper.md`（新規）** — 用語追加の申請を受け、可否を判断する。
  **既定は却下**とし、通すべき理由を申請側に示させる
  - 申請に要るもの: **語 / 説明 / 使用箇所 / 一般語で言い換えるとどうなるか**
  - **4つ目を必須にすると、申請の多くはその場で自滅する。** それが狙い
  - 却下する条件: 一般語で言い換えられる / 使用箇所が1〜2箇所 / 既存語と重なる /
    説明が書けない / その文書だけの都合 / **英語をそのまま訳しただけ**
  - **却下だけを返さない。代わりに使う語を必ず示す**
- **利害を分離する**: **語を使いたい人が可否を決めてはいけない。**
  メインは「今書いている文書を通したい」動機を持ち、自分の申請を却下しにくい

docs 影響: あり（diagrams/01_全体アーキテクチャ図.md・background/01_統合前後の差異.md・README.md —
エージェント一覧に `glossary-keeper` を追加 / templates/base/CLAUDE.md — サブエージェント節に1項目 /
templates/unity/CLAUDE.section.md — rules の案内に `docs.md` を追加）

> エージェントを1本増やしたので、**列挙している文書を grep で洗い出して全部当てた**
> （`diagrams/01` / `background/01` / `README.md`）。**思い出しでは足りない**（本ファイル冒頭の書式規約）。

## [0.13.0] — 日本語校正に「思い出す機会」を用意する

**`proofread-ja` は一度も起動していなかった。** 一方その期間に
「リリースノートを利用者目線へ書き直す」「確認文が実態より強い内容だったのを直す」
といった**校正そのものの作業が手で何度も行われていた**（ProjectTemplete の利用実績監査・2026-08-17）。

**仕組みが無いのではなく、思い出す機会が無かった。**
`coding-specialist`（H25）と同じ形だが、**こちらはユーザーが明示的に呼ぶ設計**なので、
自動起動ではなく**案内を出す**形にする。

### Changed

- **`update-docs` Step 5** — 更新した文書に**利用者が読む日本語**が含まれるとき、
  報告の末尾に `/harness-core:proofread-ja` の案内を添える。
  **当てはまらないなら添えない**（毎回出すと読まれなくなる）
- **`complete-feature` Step 4** — 完了処理の報告に同じ案内を追加。
  **機能の完了は利用者向けの文言が確定する時点**であり、ここを逃すと校正されないまま出る
- **`templates/base/CLAUDE.md`** — スキル表に「いつ呼ぶか」を明記

> **自動では走らせない。** 校正は全文精読で、走らせるかはユーザーが決める。
> **ただし案内は出す** — 出さないと呼ばれないことが実測で分かっている。

docs 影響: あり（templates/base/CLAUDE.md — スキル表の1行）

> `docs/guide/` は据え置き。**スキルの顔ぶれもフローも変わっていない**（報告に1行足しただけ）。
...
`tests/` に足して CI で毎回回す（**配布物ではないので版番号は動かない**）。

### Added

- **`tests/wiring.test.mjs`** — 6つの検査
  1. **配布するエージェントにコメント外からの呼び出しがあるか**（H25）
  2. 配布するスキルが `CLAUDE.md`（base / 環境セクション）に列挙されているか
  3. 散文中の `/<plugin>:<skill>` が実在するか
  4. **「自動実行」と主張しているスキルに実際の呼び出しがあるか**（H24）
  5. 全エージェントが `model` を持ち、既定 `sonnet`（例外は明示リストのみ）
  6. 検査そのものの検査（`stripComments` が効いているか）

### 判定式を3回直した — 検査自体の偽陽性・偽陰性

**「機械チェックを書くときは、チェック自体の偽陽性に注意する」（H14）が、そのまま起きた。**
H24 / H25 を再現した複製で**両方向**（本体は通る・事故は落ちる）を確認するまでに3回直している。

| # | 誤り | 直し方 |
|---|------|--------|
| 1 | **一覧表の「言及」を呼び出しと数えていた。** `templates/*/CLAUDE.md` のスキル表には全スキルが載るので、**どんなスキルでも「呼び出しあり」になり検査が死ぬ** | 呼び出し元は**スキルの本文だけ**に絞る |
| 2 | **素の名前の言及を呼び出しと数えていた。** 「`update-docs` / `sync-check` で設計書をまとめて更新した後」のような**時期の説明**まで拾っていた | **名前空間つきの起動形式**（`/harness-core:sync-check`）だけを呼び出しと認める |
| 3 | **「X が Y を自動で呼ぶ」の X（呼ぶ側）を呼ばれる側と取り違えていた**（本体で偽陽性2件） | 格助詞「**が**」が続く参照は**動作主**として除く |

> **`` をテンプレートリテラルに直書きしてバックスペース文字になる**という取りこぼしもあった。
> **「検査が通った」ではなく「事故を再現して落ちること」で確かめる**のが唯一の担保になる。

docs 影響: なし（`tests/` はリポジトリ内部の検査。配布物・手順のいずれも変えていない）
