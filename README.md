...
**このハーネスは現時点で一般公開しておらず、公開の予定もない。個人が複数プロジェクトを
効率よく開発できることを最優先の判断基準としている。** チーム利用・複数ユーザーでの運用・
他マシンでの再現性といった、一般公開したプロダクトなら詰めるべき懸念は、**設計で厳密に
潰さず、運用でカバーする前提**で作られている箇所がある。将来チーム利用や公開を検討する
場合は、そうした箇所を洗い出し直す必要がある。
