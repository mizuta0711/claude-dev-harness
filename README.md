...
**個人での複数プロジェクト開発を想定しており、チーム利用・複数人での同時運用は
想定していない。** 以下は動作するが、堅牢性を検証していない・意図的に対処していない。

- **チームメンバー間のオンボーディング**（他の人がcloneして即座に同じ状態で使えるか）
- **複数人が同じプロジェクトを同時に触る場合の競合**（プラグイン導入状態・設定の食い違い等）
- **複数マシン間での動作の一貫性**（同一ユーザーが別PCで開いたときの再現性）

チームでの利用や複数人での運用を検討する場合は、上記を個別に検証すること。
